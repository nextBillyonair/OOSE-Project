...
myUserID = UserHelperFuns.createUser2(my_test_client, my_socket_client)
...
test_clients[0].post('v1/tags', data=json.dumps({
    'userID':users[0][0], 
    'tags':['','',''] 
}))

test_clients[1].post('v1/tags', data=json.dumps({
    'userID':users[1][0], 
    'tags':['','',''] 
}))

test_clients[2].post('v1/tags', data=json.dumps({
    'userID':users[2][0], 
    'tags':['','',''] 
}))

# Request
test_clients[3].post('v1/tags', data=json.dumps({
    'userID':users[3][0], 
    'tags':['AI','ML','NLP'] 
}))

# Request
test_clients[4].post('v1/tags', data=json.dumps({
    'userID':users[4][0], 
    'tags':['','',''] 
}))

# Long Distance Chat
test_clients[5].post('v1/tags', data=json.dumps({
    'userID':users[5][0], 
    'tags':['','',''] 
}))


test_clients[6].post('v1/tags', data=json.dumps({
    'userID':users[6][0], 
    'tags':['Football','Engineering','Spanish'] 
}))

test_clients[7].post('v1/tags', data=json.dumps({
    'userID':users[7][0], 
    'tags':['OOSE','Observer','Courtouis'] 
}))

test_clients[8].post('v1/tags', data=json.dumps({
    'userID':users[8][0], 
    'tags':['Economics','Supply','Demand'] 
}))

# Chat 
test_clients[9].post('v1/tags', data=json.dumps({
    'userID':users[9][0], 
    'tags':['Cooking','Pwasta','Beef Wellington'] 
}))

msg = "Wanna talk?"
UserHelperFuns.checkIfConnected(my_test_client, myUserID, users[0][0], connected=False, position=0, sid=sid)
RequestHelperFuns.sendRequest(my_test_client, myUserID, users[0][0], msg, position=0, sid=sid)
